...
func selectToolByVersion(tag string) string {
	...
	if len(parts) < 2 {
		return "export_defaults_v8.go" // Default to latest
	}
	...
	case major == 7:
		if minor < 5 {
			return "export_defaults_v7.go" // v7.0 - v7.4
		} else {
			return "export_defaults_v8.go" // v7.5+
		}
	case major >= 8:
		return "export_defaults_v8.go" // v8.0+
	default:
		return "export_defaults_v8.go" // Default to latest
	}
}