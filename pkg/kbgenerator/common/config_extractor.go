...
type ConfigExtractor struct {
	...
	// If nil, uses automatic conversion (CamelCase -> kebab-case for Go, snake_case -> kebab-case for Rust)
	...
	Output kbgenerator.ConfigDefaults
...
func NewConfigExtractor(configVarName, defaultPrefix string) *ConfigExtractor {
	return &ConfigExtractor{
		...
		Output:        make(kbgenerator.ConfigDefaults),
				...
				if configVarName == e.ConfigVarName {
					...
					if value != nil {
						...
						if configKey != "" {
							...
							e.Output[configKey] = kbgenerator.ParameterValue{
								Value: finalValue,
...
// Supports both Go (CamelCase) and Rust (snake_case) field names
// Returns empty string if no mapping is available (should use toml tags instead)
		...
		for _, match := range matches {
			...
			if key != "" && value != nil {
				if _, exists := e.Output[key]; !exists {
					e.Output[key] = kbgenerator.ParameterValue{
						Value: value,
						Type:  paramType,
					}
				}
...
func FindTidbConfigFiles(tidbRoot string) []string {
	return FindConfigFiles(tidbRoot, kbgenerator.ComponentTiDB)
}
...
func FindPdConfigFiles(pdRoot string) []string {
	return FindConfigFiles(pdRoot, kbgenerator.ComponentPD)
}
...
func FindTikvConfigFiles(tikvRoot string) []string {
	return FindConfigFiles(tikvRoot, kbgenerator.ComponentTiKV)
}
...
func FindTiflashConfigFiles(tiflashRoot string) []string {
	return FindConfigFiles(tiflashRoot, kbgenerator.ComponentTiFlash)
}
...
func FindConfigFiles(repoRoot string, component kbgenerator.ComponentType) []string {
	var files []string
	var searchPaths []string

	switch component {
	case kbgenerator.ComponentPD:
		searchPaths = []string{
			filepath.Join(repoRoot, "server", "config", "config.go"),
		}
	case kbgenerator.ComponentTiDB:
		// TiDB 7.1+ uses pkg/ directory, older versions may not
		// Try both paths
		searchPaths = []string{
			filepath.Join(repoRoot, "pkg", "config", "config.go"),
			filepath.Join(repoRoot, "config", "config.go"),
		}
	case kbgenerator.ComponentTiKV:
		searchPaths = []string{
	...
	case kbgenerator.ComponentTiFlash:
