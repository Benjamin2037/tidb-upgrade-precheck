...
func (r *HighRiskParamsRule) checkParameter(
	...
	if !r.isVersionApplicable(ruleCtx.SourceVersion, paramConfig.FromVersion, paramConfig.ToVersion) {
		// This parameter is not applicable for the current source version, skip
		return nil
	}
...
// isVersionApplicable checks if the parameter configuration is applicable for the given source version
// Returns true if:
//   - fromVersion is empty (applies to all versions), OR
//   - sourceVersion >= fromVersion (if fromVersion is specified)
//   - AND sourceVersion <= toVersion (if toVersion is specified)
func (r *HighRiskParamsRule) isVersionApplicable(sourceVersion, fromVersion, toVersion string) bool {
	...
	// If fromVersion is empty, applies to all versions (unless toVersion is specified)
	if fromVersion == "" {
		// If toVersion is also empty, applies to all versions
		if toVersion == "" {
			return true
		}
		// If toVersion is specified, check if sourceVersion <= toVersion
		return compareVersions(sourceVersion, toVersion) <= 0
	}

	// Check if sourceVersion >= fromVersion
	...
	// fromVersion is specified and sourceVersion >= fromVersion, and toVersion is empty
	return true
}
