	...
	for compType, sourceDefaults := range ruleCtx.SourceDefaults {
		...
		for paramName, sourceDefaultValue := range sourceDefaults {
			...
			if fmt.Sprintf("%v", currentValue) != fmt.Sprintf("%v", sourceDefault) {
				// User has modified this parameter
				paramType := "config"
				...
				results = append(results, CheckResult{
					...
					ParamType:     paramType,
					Severity:      "info",
					Message:       fmt.Sprintf("Parameter %s in %s has been modified from source default", displayName, compType),
					Details:       fmt.Sprintf("Current: %v, Source default: %v", currentValue, sourceDefault),
					CurrentValue:  currentValue,
					SourceDefault: sourceDefault,
					Suggestions: []string{
						"Review if this modification is intentional",
