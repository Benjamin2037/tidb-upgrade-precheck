...
// FindPlaygroundInstanceAddr finds component instance address from playground directory
// Extracts port from directory name ({component}-{port}) and constructs address as 127.0.0.1:{port}
// component should be "tikv" or "tiflash"
func FindPlaygroundInstanceAddr(component, tag string) (string, error) {
	// Validate component name
	component = strings.ToLower(component)
	if component != "tikv" && component != "tiflash" {
		return "", fmt.Errorf("unsupported component: %s (must be 'tikv' or 'tiflash')", component)
	}
	...
	// Look for directory with component prefix (e.g., "tikv-20160" or "tiflash-9000")
	prefix := component + "-"
	for _, entry := range entries {
		if entry.IsDir() && strings.HasPrefix(entry.Name(), prefix) {
			// Extract port from directory name (e.g., "tikv-20160" -> "20160")
			portStr := strings.TrimPrefix(entry.Name(), prefix)
			if portStr != "" {
				// Construct address as 127.0.0.1:{port}
				return fmt.Sprintf("127.0.0.1:%s", portStr), nil
			}
		}
	}

	// Format component name for error message (capitalize first letter)
	componentName := strings.ToUpper(component[:1]) + component[1:]
	return "", fmt.Errorf("%s instance address not found in %s", componentName, dataBaseDir)
}
