...
func NewSysVarExtractor(vardefDir string) *SysVarExtractor {
	extractor := &SysVarExtractor{
		VardefDir:    vardefDir,
		Output:       make(types.SystemVariables),
		...
		if sel, ok := compLit.Type.(*ast.SelectorExpr); ok {
			...
			if sel.Sel.Name == "SysVar" || sel.Sel.Name == "SysVarType" {
				...
				if varName != "" && varValue != "" {
					...
					e.Output[varName] = types.ParameterValue{
						Value: varValue,
						Type:  varType,
					}
				}
	...
	for _, m := range matches {
		if len(m) >= 4 {
			...
			if _, exists := e.Output[finalName]; !exists {
				...
				e.Output[finalName] = types.ParameterValue{
					Value: val,
					Type:  paramType,
				}
			}
...
// Handles version differences (before and after v7.5.0)
func FindSysVarFiles(tidbRoot, version string) []string {
	...
	if major > 7 || (major == 7 && minor >= 5) {
		...
		// TiDB < 7.5 uses sessionctx/ directory (no pkg/)
		searchPaths = []string{
