...
// - pkg/session/upgrade.go (newer versions, e.g., v7.5+)
...
func CollectUpgradeLogicFromSource(repoRoot string) (*types.UpgradeLogicSnapshot, error) {
	...
	var (
		...
		curVersion    string
		curComment    string
		results       []types.UpgradeParamChange
			...
			if strings.Contains(line, "mustExecute") {
				...
				if strings.Contains(line, "mysql.global_variables") {
					...
					if m := varNameRe.FindStringSubmatch(line); m != nil {
						...
						change := types.UpgradeParamChange{
							Version:     curVersion,
							FuncName:    curFunc,
							VarName:     varName,
							Name:        varName,
	...
	return &types.UpgradeLogicSnapshot{
