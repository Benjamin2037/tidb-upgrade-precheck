...
type TiDBCollector interface {
	Collect(addr, user, password string) (*collector.ComponentState, error)
...
func (c *tidbCollector) Collect(addr, user, password string) (*collector.ComponentState, error) {
	state := &collector.ComponentState{
		Type:      collector.TiDBComponent,
	...
	state.Variables = collector.ConvertVariablesToSystemVariables(variables)
