# TiDB Parameter Collection Operation Guide
...
The tidb and tidb-upgrade-precheck repositories need to be placed in the same parent directory. 
For example, if your parent directory is `sourcecode`, the structure should be as follows:
...
```bash
# Create the sourcecode directory (if it doesn't exist)
mkdir -p sourcecode

# Change to the sourcecode directory
cd sourcecode

# Clone the TiDB repository with all tags and branches
...
# Clone the tidb-upgrade-precheck repository
git clone https://github.com/pingcap/tidb-upgrade-precheck.git
```

Alternatively, if you already have a TiDB clone, you can fetch all tags:
...
This structure is important because the parameter collection tool expects to find the TiDB source code 
in a sibling directory by default. If you have a different directory structure, you can specify the 
TiDB source code path using the `--repo` parameter in commands.

### 1.3 Environment Verification

After setting up the directory structure, verify that both repositories are properly cloned:

```bash
# Check that both directories exist
ls -la sourcecode/

# Verify TiDB repository
cd sourcecode/tidb
git status

# Verify tidb-upgrade-precheck repository
