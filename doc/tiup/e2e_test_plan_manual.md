...
# 重要：upgrade_logic.json 会在生成知识库时自动生成（TiDB 模块的强制变更逻辑）
# upgrade_logic.json 是全局文件，包含所有历史版本的升级逻辑，kb_generator 会自动从 master/main 分支提取
# 方式 A: 使用 generate_knowledge.sh 脚本（推荐，会自动生成 upgrade_logic.json）
# bash scripts/generate_knowledge.sh --force --serial --start-from=v7.5.1 --stop-at=v7.5.1

# 方式 B: 使用 kb_generator 单独生成（会自动生成 upgrade_logic.json）
# GOWORK=off go run cmd/kb_generator/main.go \
#   --version=v7.5.1 \
...
- [ ] TiDB upgrade_logic.json 文件存在（包含强制变更逻辑）
  ...
  if [ -f "knowledge/tidb/upgrade_logic.json" ]; then
    ...
    echo "  Note: upgrade_logic.json is automatically generated by kb_generator when generating knowledge base"
    echo "  Run: GOWORK=off go run cmd/kb_generator/main.go --version=v8.5.4 --tidb-repo=<tidb-repo-path> --components=tidb"
  fi
  ```
